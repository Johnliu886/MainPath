...
extern int mainpaths(wchar_t *, wchar_t *, wchar_t *, wchar_t *, int, int, double, double, int, double, double, int, int, int, int, double, wchar_t *, wchar_t *, int, struct ATABLE *, wchar_t *, wchar_t *, int);
...
extern int test_eI();
extern int readwrite_WOS(wchar_t *, wchar_t *, int);
extern int readwrite_Scopus(wchar_t *, wchar_t *, int);
				  ...
				  int Neg4star, int Neg3star, int Neg2star, int Neg1star, int take_weight, wchar_t *sversion)
{
	...
	// valid until 2014/08
	if (st.wYear > 2014 || (st.wYear == 2014 && st.wMonth > 8))
	...
	_wfopen_s(&logstream, logfname, L"w");
#ifdef MDEBUG
...
//#######
	...
	{	
		...
		if (_wfopen_s(&wstream, frecordname, L"r") != 0)	
...
//#endif STUDENT_VERSION
	...
	if (full_record_type == USPTO_DATA || full_record_type == THOMSON_INNOVATION_DATA || full_record_type == PGUIDER_DATA || full_record_type == WUHAN_DATA)	// citation files for these data are created automatically, not given
	...
	if (_wfopen_s(&astream, atablefname, L"r") == 0)	// alias table is provided
	{
		nlines = 0;
		while (TRUE)
		{		
		...
		{		
			...
			if (line[0] == '\n' || line[0] == '\r')
				continue;
			swscanf_s(line, L"%s %s", atable[nlines].truename, MAX_NODE_NAME-1, atable[nlines].alias, MAX_NODE_NAME-1);
			nlines++;	
		}
	}
	nalias = nlines;
	...
	if (full_record_type == WOS_DATA)
	{
		...
		readwrite_WOS(frecordname, tmpname, 99999);	// 99999 indicates write all nodes
	}
	else if (full_record_type == SCOPUS_DATA)
	{
		swprintf(tmpname, L"%s N%d.csv", tname, nnodes);
		readwrite_Scopus(frecordname, tmpname, 99999);
	}
...
//============
	...
	ret2 = mainpaths(frecordname, tname, branchname, gfname, spmethod, mptype, mtie_tolerance, mnpths, 
		...
		n_top_edges_local, n_top_edges_global, decay, clanfilename, dpath, take_weight, atable, rpajekname, gfname, groupfinderoptions);
	fflush(logstream);
...
//	
	...
	if (_wfopen_s(&istream, inpname, L"r") != 0)
	...
	if (make_atable == TRUE)
	{
		...
		if (_wfopen_s(&aliasstream, aliasname, L"w") != 0)
			return MSG_OFILE_CANNOTWRITE;
		for (i = 0; i < nnd; i++) 
...
// Open the given citation file (will fail if the file does not exist)
	if (_wfopen_s(&fstream, fname, L"r") != 0)
		return MSG_IFILE_NOTFOUND;
