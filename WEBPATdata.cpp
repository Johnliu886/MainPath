...
//
// Revision History:
// 201/03/10 1.00	Basic function works
//
...
int parse_WEBPAT_1st_line(wchar_t *, int, wchar_t *);
int parse_WEBPAT_line(wchar_t *, int, wchar_t *);
int parse_WEBPAT_Patent_Ref(wchar_t *, wchar_t *, FILE *);
int parse_store_WEBPAT_inventors(wchar_t *, int *, struct AUTHORS *);
int parse_WEBPAT_inventor_names(wchar_t *, int *, int *, int, struct AUTHORS *);
int parse_WEBPAT_IPC(wchar_t *, int *, wchar_t *);
int parse_WEBPAT_UPC(wchar_t *, int *, wchar_t *);
int get_WEBPAT_number_inventors(wchar_t *);
...
//	
	...
	i_ut = parse_WEBPAT_1st_line(line, SP_TAB, ut);	// document ID
	i_au = parse_WEBPAT_1st_line(line, SP_TAB, au);	// author names
	i_py = parse_WEBPAT_1st_line(line, SP_TAB, py);	// year of publication
	i_cr = parse_WEBPAT_1st_line(line, SP_TAB, L"Patent_Ref");	// this field actually contains "citing" information
	i_ipc = parse_WEBPAT_1st_line(line, SP_TAB, L"IPC");	// IPC code
	i_usc = parse_WEBPAT_1st_line(line, SP_TAB, L"UPC");	// US Class code 
	i_an = parse_WEBPAT_1st_line(line, SP_TAB, an);	// Assignee/Applicant
	...
	{		
		...
		parse_store_WEBPAT_inventors(tfield, &naus, authors);
	...
	{		
		...
		if (tfield[0] == '\0')	// empty author name, assign an artificial name to it
			swprintf(tfield, LBUF_SIZE, L"Anonymous %d", i);
		uspto[i].ninventor = get_WEBPAT_number_inventors(tfield);
		parse_WEBPAT_inventor_names(tfield, &uspto[i].ninventor, uspto[i].inventor, naus, authors);
#ifdef DEBUG
		...
		parse_WEBPAT_Patent_Ref(tfield, uspto[i].pid, cstream);
		parse_WEBPAT_line(line, i_ipc, tfield);
		parse_WEBPAT_IPC(tfield, &uspto[i].nipc, uspto[i].ipc);
		parse_WEBPAT_line(line, i_usc, tfield);
		parse_WEBPAT_UPC(tfield, &uspto[i].nusc, uspto[i].usc);
		parse_WEBPAT_line(line, i_an, tfield);
		wcscpy(uspto[i].assignee, tfield);
		i++;	
	}

	fclose(cstream);	// close citation file (relationship file)
...
//
int parse_WEBPAT_1st_line(wchar_t *line, int separator, wchar_t *tfname)
{
	wchar_t ch, *sp, *tp, *fp;
...
//
int parse_store_WEBPAT_inventors(wchar_t *astr, int *nau, struct AUTHORS *au)
{
	wchar_t ch, *sp, *tp;
...
//
int parse_WEBPAT_Patent_Ref(wchar_t *str, wchar_t *pid, FILE *stream)
{
	int i, state;
	int cnt;
	int issued;
...
//
int parse_WEBPAT_IPC(wchar_t *str, int *nipc, wchar_t *ipc_code)
{
	int i, state;
	int cnt;
	wchar_t *sp, *tp, ch;
...
//
int parse_WEBPAT_UPC(wchar_t *str, int *nusc, wchar_t *usc_code)
{
	int i, state;
	int cnt;
	wchar_t *sp, *tp, ch;
...
//
int get_WEBPAT_number_inventors(wchar_t *allnames)
{
	wchar_t ch, *sp;
	int cnt;
...
//
int parse_WEBPAT_inventor_names(wchar_t *astr, int *nau, int *au, int nathrs, struct AUTHORS *athrs)
{
	wchar_t ch, *sp, *tp;
	wchar_t tmps[BUF_SIZE];
	int tnau;
