...
int find_shortest_paths(int nn, struct FWDIST *fw, short *fw_midpt, int ns, int *st, int ne, int *end, int *npaths, struct SPATH *q)
{
	...
	for (i = 0; i < nn; i++) 
	{
		for (j = 0; j < nn; j++)
		{
			...
			fwprintf(logstream, L"[%s->%s %.2f:%d] ", nw[i].name, nw[j].name, tmp, fw_midpt[i*nn+j]+1);
		}
	...
	for (i = 0; i < 10; i++)
	{
		if (tfw[i].dist == (float)HUGE_NUMBER) tmp = -1; else tmp = tfw[i].dist;
		fwprintf(logstream, L"{%s->%s %.2f:%d}\n", nw[tfw[i].i].name, nw[tfw[i].j].name, tmp, tfw_midpt[i]+1);
	}
#endif DEBUG
...
int floyd_warshall(int nn, double *cmat, struct FWDIST *fw, short *fw_midpt)
{
	...
	for (i = 0; i < nn; i++) 
	{
		...
		{
			if (cmat[i*nn+j] != 0.0)
				fw[i*nn+j].dist = (float)cmat[i*nn + j];
			else
				fw[i*nn+j].dist = (float)HUGE_NUMBER; // no direct link
			if (i == j)  // diagonal elements
				fw[i*nn+j].dist = 0.0;
			if (fw[i*nn+j].dist != 0.0 && fw[i*nn+j].dist < (float)HUGE_NUMBER)
				fw_midpt[i*nn+j] = i;
		}
	}
	...
	for (k = 0; k < nn; k++)
	{
		for (i = 0; i < nn; i++) 
		{
			for (j = 0; j < nn; j++) 
			{
				if (fw[i*nn+k].dist == (float)HUGE_NUMBER || fw[k*nn+j].dist == (float)HUGE_NUMBER)	// if one of the link has no connection, don't bother
					continue;
				if (fw[i*nn+j].dist > (fw[i*nn+k].dist + fw[k*nn+j].dist)) 
				{			
					fw[i*nn+j].dist = fw[i*nn+k].dist + fw[k*nn+j].dist;
					fw_midpt[i*nn+j] = k; 
				}
			}
		}
